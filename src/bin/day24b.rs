...
impl Expr {
    ...
    fn simplify_mod(self: &Rc<Self>, divisor: i64) -> Rc<Self> {
        match &**self {
            Expr::Literal(x) => Rc::new(Expr::Literal(x % divisor)),
            Expr::Add(x, y) => Rc::new(Expr::Add(x.simplify_mod(divisor), y.simplify_mod(divisor))),
            Expr::Mul(x, y) => Rc::new(Expr::Mul(x.simplify_mod(divisor), y.simplify_mod(divisor))),
            _ => self.clone(),
        }
    }
        ...
        match &**self {
            ...
            Expr::Mod(a, b) => {
                ...
                match (&*a2, &*b2) {
                    ...
                    (_, &Expr::Literal(bv)) => {
                        Rc::new(Expr::Mod(a2.simplify_mod(bv), Rc::new(Expr::Literal(bv))))
                    }
                    _ => {
                        if Rc::ptr_eq(&a, &a2) && Rc::ptr_eq(&b, &b2) {
                            self.clone()
    ...
    for line in INPUT.lines() {
        ...
        match parts.as_slice() {
            ...
            ["add", a, b] => {
                ...
                registers[lvalue(a)] = Rc::new(Expr::Add(av, bv)).simplify(15, &[]);
            }
            ["mul", a, b] => {
                let av = rvalue(a, &registers);
                let bv = rvalue(b, &registers);
                registers[lvalue(a)] = Rc::new(Expr::Mul(av, bv)).simplify(15, &[]);
            }
            ["div", a, b] => {
                let av = rvalue(a, &registers);
                let bv = rvalue(b, &registers);
                registers[lvalue(a)] = Rc::new(Expr::Div(av, bv)).simplify(15, &[]);
            }
            ["mod", a, b] => {
                let av = rvalue(a, &registers);
                let bv = rvalue(b, &registers);
                registers[lvalue(a)] = Rc::new(Expr::Mod(av, bv)).simplify(15, &[]);
            }
            ["eql", a, b] => {
                let av = rvalue(a, &registers);
                let bv = rvalue(b, &registers);
                registers[lvalue(a)] = Rc::new(Expr::Eql(av, bv)).simplify(15, &[]);
            }
            _ => panic!("Unknown: {:?}", parts),
        }
    }
    let res = rvalue("z", &registers).simplify(100, &[]);
    println!("{}", res);
